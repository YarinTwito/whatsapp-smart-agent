...
def test_route_after_validation(llm_service):
    ...
    assert llm_service.route_after_validation(invalid_state) == "handle_invalid_document"
...
def test_generate_response(llm_service):
    ...
    state = State(
        document_valid=True,
        messages=[Message(role="user", content="end")]
    )
    result = llm_service.generate_response(state)
    assert result == state
    ...
    with patch.object(llm_service._vectorstores["test.pdf"], 'as_retriever', return_value=mock_retriever):
        with patch('langchain_core.output_parsers.StrOutputParser.invoke', return_value="Test response"):
            result = llm_service.generate_response(state)
...
async def test_get_answer(llm_service):
    ...
    with patch.object(llm_service._vectorstores["test_id"], 'as_retriever', return_value=mock_retriever):
        with patch('langchain_core.output_parsers.StrOutputParser.invoke', return_value="Test answer"):
            result = await llm_service.get_answer("test question", "test_id")
            assert result["answer"] == "Test answer"
    
    # Test with exception
    with patch.object(llm_service._vectorstores["test_id"], 'as_retriever', side_effect=Exception("Test error")):
        result = await llm_service.get_answer("test question", "test_id")
        assert "error" in result["answer"].lower()
